class PullRequestMonitor
  ...
  def process_prs
    GithubService.call(:user => repo.upstream_user, :repo => repo.name) do |github|
      @github = github
  ...
  def process_pr
    branch_name = git.pr_branch_name(pr.number)
  ...
  def create_pr_branch_record(branch_name)
    ...
    repo.branches.create!(:name => branch_name, :last_commit => last_commit, :commit_uri => commit_uri, :pull_request => true)
  end
